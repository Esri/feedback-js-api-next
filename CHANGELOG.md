...
APIs and WebGL2 features (where supported). Image decoding is now threaded and asynchronous.
We're also using texture immutability to speed uploads, and chunking to partially upload
textures over multiple frames without blocking the main thread. With these changes, maps
using multiple MapImageLayers will feel more responsive and have higher framerates when panning.
